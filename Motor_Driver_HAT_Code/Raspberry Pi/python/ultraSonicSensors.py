...
import time
import threading
from gpiozero import DistanceSensor  # Import DistanceSensor from gpiozero
...
def measure_distance(trigger_pin, echo_pin):
    ...
    pulse_start = time.time()  # Initialize pulse_start here
    pulse_end = pulse_start  # Initialize pulse_end
...
def read_left_sensor(queue_len):
    ...
    sensor_l = DistanceSensor(echo=ECHO_PIN_LEFT, trigger=TRIG_PIN_LEFT, queue_len=queue_len, pin_factory=gpio_factory)
    while True:
...
def read_right_sensor(queue_len):
    ...
    sensor_r = DistanceSensor(echo=ECHO_PIN_RIGHT, trigger=TRIG_PIN_RIGHT, queue_len=queue_len, pin_factory=gpio_factory)
    while True:
